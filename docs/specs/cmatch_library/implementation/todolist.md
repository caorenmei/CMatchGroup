...
- [ ] 5.1 梳理 `exception_handling.md` 中的实体数据修复与赛季时间修复规则。
- [ ] 5.2 根据当前代码库状态（已存在服务实现）微调需求与验收条件。
- [ ] 5.3 在 `BuildSeason` 中实现“当前时间不在赛季内”凭据的数据修复。
- [ ] 5.4 实现赛季时间不一致时的修复逻辑（以配置时间为准）。
- [ ] 5.5 编写异常场景测试：跨赛季凭据修复、时间戳修复、合服后分组 ID 冲突修复。
- [ ] 5.6 应用 `clang-format` 与 `clang-tidy`。
- [ ] 5.7 提交变更，更新本文件状态。
...
- [ ] 6.1 梳理所有规格文档，确认无遗漏的公共 API 与行为。
- [ ] 6.2 根据当前代码库状态微调集成测试范围与文档更新清单。
- [ ] 6.3 编写端到端集成测试：配置加载 → 提交凭据 → 报名 → 切换赛季 → 查询结算。
- [ ] 6.4 更新项目根 `README.md`，补充构建说明、库功能简介与快速开始。
- [ ] 6.5 为新增公共头文件补充 Doxygen 风格简体中文注释。
- [ ] 6.6 全量构建：`cmake --build --preset ninja-debug`。
- [ ] 6.7 全量测试：`ctest --preset ninja-debug`。
- [ ] 6.8 全量格式与静态分析检查。
- [ ] 6.9 提交变更，更新本文件状态。
